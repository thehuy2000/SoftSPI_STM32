...
uint8_t Soft_SPI_TransferByte(uint8_t byte);
...
int main(void)
{
  ...
  {	
		uint8_t rev;
		/*Slave Write*/
		rev = Soft_SPI_TransferByte('A'); // 0x41			65
  }

}

/*SPI Transmit byte*/
	uint8_t Soft_SPI_TransferByte(uint8_t byteTx){ 
		uint8_t byteRx = 0;
		uint8_t ibit, temp;
		while(HAL_GPIO_ReadPin(GPIOB, SLAVE_CS_Pin) == SET);
		for(int ibit = 0x80; ibit > 0; ibit = ibit >> 1)
	 {
		  temp = byteTx & ibit;
			HAL_GPIO_WritePin(GPIOB, SLAVE_MISO_Pin, temp);
		 
		  while(HAL_GPIO_ReadPin(GPIOB, SLAVE_SCK_Pin) == 0);
		 
		  if(HAL_GPIO_ReadPin(GPIOB,SLAVE_MOSI_Pin) == 1)
			  byteRx = byteRx | ibit;
		 
		  while(HAL_GPIO_ReadPin(GPIOB, SLAVE_SCK_Pin) == 1);
	 }
	 return byteRx;
 }

/*SPI Transfer frame data*/
void Soft_SPI_Transfer(char *buff){
	signed char maxSize = (signed char)strlen(buff);
	HAL_GPIO_WritePin(GPIOA, MASTER_CS_Pin, GPIO_PIN_RESET);
	for(int i = 0; i<maxSize; i++)
	{
		Soft_SPI_TransferByte(buff[i]);
	}
	HAL_GPIO_WritePin(GPIOA, MASTER_CS_Pin, GPIO_PIN_SET);
}	
